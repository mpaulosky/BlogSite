...
public class PgArticleRepository : IArticleRepository
{
	...
	public async Task ArchiveArticle(string slug)
	{
		...
		PgArticle? post = await _context.Articles.FirstOrDefaultAsync(p => p.Slug == slug);
	...
	public async Task<PgArticle?> GetArticle(string dateString, string slug)
	{
		...
		PgArticle[] theArticles = await _context.Articles
				.AsNoTracking()
				.Where(p => p.Slug == slug)
				.Select(p => p)
				.ToArrayAsync();
	...
	public async Task<IEnumerable<PgArticle>> GetArticles()
	{
		...
		PgArticle[] posts = await _context.Articles.AsNoTracking().ToArrayAsync();

		return posts.Select(p => p);
	}
